...
//    LET'S WORK FOR AVAILABLE BUSES FORM FIRST :
    ...
    public void availableBusDelete(){
        ...
        try{
            ...
            if (availableB_busID.getText().isEmpty()
                    || availableB_location.getText().isEmpty()
                    || availableB_status.getSelectionModel().getSelectedItem() == null
                    || availableB_price.getText().isEmpty() || availableB_date.getValue() == null)
            {
                alert = new Alert(AlertType.ERROR);
                alert.setTitle("Error Message");
                alert.setHeaderText(null);
...
//    NOW LET'S CREATE TABLE FOR customer:
...
//            CHECK IF EMPTY
            if(bookingTicket_sci_firstName.getText().isEmpty()
                ...
                alert.showAndWait();
//                NEED TO REMOVE THE SEAT# IF THE CUSTOMER IS ALREADY CHOSE THAT
...
//                WE NEED TO INSERT THE DATA ON CUSTOMER_RECEIPT FOR OUR RECEIPT :
    ...
    public void bookingTicketReceipt(){
        ...
        try{
            if(totalP > 0){
                JasperDesign jDesign = JRXmlLoader.load("D:\\OneDrive\\Desktop\\Semester Notes\\Second Semester\\Java CLass Lectures\\Project\\BusManagementSystem-main\\busTicketBookingManagementSystem\\src\\busticketbookingmanagementsystem\\report.jrxml");
...
//            TO UPDATE THE FORM ONCE YOU CLICK THE AVAIALABLE BUSES BUTTON 
            ...
            customersSearch();
//            LET'S PROCEED TO OUR DASHBOARD FORM :
        }
    }
...
//NOW LET'S PROCEED TO OUR CHART
